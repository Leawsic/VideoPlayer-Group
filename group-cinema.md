...
重新进入区域后，如果本地已缓存房间当前视频，客户端会按最新同步状态或房间进度恢复播放；后续仍以后端推送的 `room_state` / `sync_state` 为准。

## 同步机制

Room Server 作为播放时钟源，并向客户端推送 `room_state` / `sync_state`。

成员客户端收到后：

- 如果未绑定屏幕或绑定屏幕不可用则只保存状态。
- 如果本地没有播放，会尝试用 `GroupRoomState.currentVideo` 恢复播放。
- 如果正在播放，会比较后端权威进度和本地进度。
